...
def extract_subtopics_bertopic(texts, embed_model, min_cluster_size=BERTOPIC_MIN_CLUSTER_SIZE, embed_batch_size=32, banned_words: Optional[List[str]] = None):
    ...
    custom_pattern = r"\b(?!\d+\b)\w\w+\b"
    vectorizer_model = CountVectorizer(ngram_range=(1, 2), token_pattern=custom_pattern, stop_words=banned_words) if banned_words is not None else None
    hdbscan_model = HDBSCAN(
                    min_cluster_size=15, # 원하는 값으로 조정
                    min_samples=8,
                    metric='euclidean', 
    ...
    bm_kwargs = dict(language = "multilingual", embedding_model=None, calculate_probabilities=False, verbose=False, min_topic_size=min_cluster_size)
    ...
    if hdbscan_model is not None:
        bm_kwargs['hdbscan_model'] = hdbscan_model
...
def main(args):
    ...
    if not (args.cleaned):
        ...
        df['article_cleaned'] = df['combined_text'].astype(str).tolist()
    ...
    # 2-2) If already cleaned, just load cleaned
        ...
        if 'combined_text' not in df.columns:
            df['combined_text'] = df['head'].astype(str) + ". " + df['subhead'].astype(str) + ". " + df['content_parsed'].astype(str)
    ...
    texts_for_topics = df['combined_text'].to_list()
    ...
    if getattr(args, 'reduce_to', None) is not None and int(args.reduce_to) > 0:
        try:
            ...
            topic_model = topic_model.reduce_topics(texts_for_topics, nr_topics=target)
            # new_topics = topic_model.reduce_outliers(texts_for_topics, topic_model.topics_, strategy = "c-tf-idf")

            # update assignments in the original dataframe alignment
            df.loc[mask_nonregional, 'subtopic'] = topic_model.topics_
        except Exception as e:
    ...
    # fig_path = args.output.rsplit('.',1)[0] + "_fig.html"
    # fig_topics.write_html(fig_path)
    ...
    edu_df = df[['id', 'head', 'subhead', 'pubtime', 'medium_name', 'content_parsed', 'subtopic']].copy()
    ...
    # Save output
    outcols = ['id', 'head', 'subhead', 'pubtime', 'medium_name', 'content_parsed','subtopic', 'subtopic_name', 'subtopic_label']
    ...
    pivoted_df = edu_df.pivot_table(index='subtopic_name', values='subtopic', aggfunc='count')
